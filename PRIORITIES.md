# PhxMediaLibrary priorities and roadmap

This document tracks what to build next, ordered by what matters most: developer experience. The principle behind every item is the same. A feature earns its place only if it cuts repetitive work for the developer using the library.
...
- [Vision](#vision)
- [Milestone 1: LiveView media component (v0.5.1) ✅](#milestone-1-liveview-media-component-v051-)
- [Milestone 2: make the core solid ✅](#milestone-2-make-the-core-solid-)
- [Milestone 3a: error handling and validation hardening (v0.3.0) ✅](#milestone-3a-error-handling-and-validation-hardening-v030-)
- [Milestone 3b: remote URLs, metadata and Oban enhancements (v0.4.0) ✅](#milestone-3b-remote-urls-metadata-and-oban-enhancements-v040-)
...
When an Elixir or Phoenix developer needs to associate files with database records, PhxMediaLibrary should be the first thing they reach for. The bar is Spatie's Laravel Media Library, adapted for how Elixir and Phoenix actually work.

A developer should go from zero to a working media upload UI in a few minutes:
...
Everything else, conversions, responsive images, S3 storage, layers on top without rewriting anything.

---

## Milestone 1: LiveView media component (v0.5.1) ✅

**Goal**: ship a ready-to-use LiveView component that removes 150+ lines of upload boilerplate.

**Status**: complete. All core sub-items are implemented. The `MediaLive` LiveComponent gives the zero-boilerplate "drop one component" experience. The lower-level `<.media_upload>`, `<.media_gallery>`, and `LiveUpload` helpers give full control for custom UIs. Only drag-and-drop reordering (`.MediaSortable`) is deferred to Milestone 4.

This is the single highest-impact feature for adoption. Every Phoenix developer who handles file uploads writes the same code over and over. This removes it.

### 1.0: `PhxMediaLibrary.MediaLive`, zero-boilerplate LiveComponent *(v0.5.1)*

- [x] Self-contained LiveComponent that encapsulates the upload and gallery lifecycle
...
- [x] Sends `{PhxMediaLibrary.MediaLive, {:uploaded, collection, media_items}}` and `{PhxMediaLibrary.MediaLive, {:deleted, collection, media}}` messages to the parent
...
**Status**: ✅ Done (v0.5.1). This is the recommended approach for most use cases. For full control, use the lower-level `<.media_upload>` plus `<.media_gallery>` plus `LiveUpload` helpers below.
...
- [x] Emits events the parent LiveView can hook into (`:media_added`, `:media_removed`, `:media_error`) via the `:notify` option on `consume_media/5` and `delete_media_by_id/2`. When set to a pid, sends `{:media_added, media_items}`, `{:media_removed, media}`, or `{:media_error, reason}` to the process for `handle_info/2`
...
- [x] `media_upload_errors/1`, `media_entry_errors/2` are human-readable error helpers
- [x] `has_upload_entries?/1`, `image_entry?/1` are introspection helpers
...
- [x] `.MediaDropZone` handles drag-and-drop with visual states (idle, hover, active, dropped flash)
- [x] `.MediaPreview` is handled natively by `<.live_img_preview>` (no custom hook needed)
- [ ] `.MediaSortable` for drag-and-drop reordering (deferred to Milestone 4, needs reorder API)
- [x] `.MediaProgress` is a CSS transition-based smooth progress bar (no JS hook needed)

### 1.5: default styles

- [x] Ship Tailwind CSS classes that work without configuration
- [x] Respect the user's design system. Components are customizable, not opinionated (all components accept `:class`, `:item` slot, `:drop_zone` slot for full override)
- [x] Provide defaults that look good without any customization
...
- [x] `<.media_upload_button>` is a compact inline variant for embedding in forms
- [x] `phx-drop-target-active:` custom variant documented for consumer app CSS setup

### 1.6: multi-file selection and progress bar fixes *(v0.5.1)*

- [x] Non-`single_file` collections default to `max_entries: 10` (enables the `multiple` attribute on the file input)
...
**Status**: complete. All sub-items are implemented, 370 tests passing (297 unit + 17 Oban worker + 56 integration).

### 2.1: make `:image` (libvips) optional

- [x] Move `{:image, "~> 0.54"}` to an optional dependency
- [x] Core library must compile and work without it (file storage, collections, DB persistence)
- [x] Image processing features degrade when `:image` is not available
- [ ] Add `PhxMediaLibrary.ImageProcessor.Mogrify` as an alternative adapter (also optional)
- [x] Clear error messages when a user tries to use conversions without an image processor installed
- [x] Wrap all optional-dep modules in `if Code.ensure_loaded?` compile guards (v0.5.1, S3 adapter added)

**Status**: ✅ Done (except the Mogrify adapter, deferred to a future milestone). `:image` is `optional: true` in `mix.exs`. `ImageProcessor.Image` is wrapped in `if Code.ensure_loaded?(Image)`. `ImageProcessor.Null` returns clear error messages that guide the developer. `Config.image_processor/0` auto-detects the available backend.

**v0.5.1 update**: `Storage.S3` is now also wrapped in `if Code.ensure_loaded?(ExAws.S3)`, matching the pattern used by `ImageProcessor.Image` and `AsyncProcessor.Oban`. Consumer projects without S3 deps no longer see roughly 15 noisy `ExAws.S3.* is undefined` compile warnings.

**Why**: many users just need file attachments (PDFs, CSVs, documents). Requiring a libvips install to store a PDF is a dealbreaker.

### 2.2: integration tests with a real database

- [x] Wire up `TestRepo` with `Ecto.Adapters.SQL.Sandbox`
- [x] Add a migration for the test database
...
- [x] Look up the model's `media_conversions/0` and match by name to get the full config
- [x] Add proper error handling and logging
- [x] Add a test for the Oban worker (using `Oban.Testing`)
- [x] Extract `find_model_module/1` into `PhxMediaLibrary.ModelRegistry` (v0.5.1), always compiled, no Oban dependency

**Status**: ✅ Done. 17 dedicated tests in `test/phx_media_library/workers/process_conversions_test.exs` use `Oban.Testing.perform_job/3`. The `Workers.ProcessConversions` worker now stores `mediable_type` in the job args, discovers the originating schema module via `ModelRegistry.find_model_module/1` (using a persistent_term cache), retrieves full `Conversion` definitions from the model's `get_media_conversions/1` or `media_conversions/0`, and filters by requested names. It handles legacy job args. It logs for missing media, unresolvable modules, and empty conversion lists. Tests cover missing-media discard, conversion resolution from TestPost (full definitions with dimensions/quality/fit), collection-scoped conversions, legacy args fallback, unknown mediable_type fallback, model module discovery and caching, explicit model registry, and job changeset construction.

**v0.5.1 update**: the model discovery logic (`find_model_module/1`, `get_model_conversions/2`, persistent_term cache) moved into `PhxMediaLibrary.ModelRegistry`, an always-compiled module with no optional dependencies. The Oban worker and `mix phx_media_library.regenerate` both delegate to it. `ProcessConversions.find_model_module/1` stays as a `defdelegate` for backwards compatibility.
...
### 2.4: hybrid `has_many` and query helpers
...
- [x] Consider whether `has_many` can use the `:where` option for collection-scoped associations

**Status**: ✅ Done. `has_media()` injects a polymorphic `has_many :media` using `Ecto.Schema.__has_many__/4` directly (bypassing macro-expansion timing issues with Ecto's schema block). It uses `:where` for `mediable_type` scoping and `:defaults` for auto-populating on build. `has_media(:images)` creates a collection-scoped `has_many :images` with both `mediable_type` and `collection_name` in the `:where` clause. The media type is resolved at module compilation time (not macro expansion time) from `@ecto_struct_fields` or the explicit override. `PhxMediaLibrary.media_query/2` provides composable Ecto queries. Integration tests verify `Repo.preload(post, [:media, :images, :documents, :avatar])`.
...
- [x] Compute a SHA-256 checksum during upload (before storage)
- [x] Store the checksum in the database
- [x] Add `Media.verify_integrity/1` to check the stored file against the checksum
- [x] Update the migration template
- [ ] Consider using the checksum for deduplication (optional)

**Status**: ✅ Done (except deduplication, deferred to the Parking lot). The `Media` schema has `checksum` and `checksum_algorithm` fields. `MediaAdder.store_and_persist/4` computes SHA-256 before storage via `Media.compute_checksum/2` (supports sha256, sha1, md5). `Media.verify_integrity/1` re-reads from storage and compares. Migration `20240101000002_add_checksum_to_media.exs` adds the columns and an index. Integration tests verify correct checksum storage, untampered file verification, and tamper detection.

**Why**: essential for data integrity, especially with cloud storage.

### 2.6: fix polymorphic type derivation

- [x] Replace naive `Module.split |> List.last |> append "s"` with a configurable approach
- [x] Option 1: let a schema define `@media_type "blog_posts"` via `HasMedia`
- [x] Option 2: use the full module name, underscored (e.g., `"my_app/blog/post"`)
- [x] Option 3: derive from the Ecto schema source (table name), most reliable ← **chosen**
- [x] Whichever we pick, make it overridable per-schema
- [ ] Add a migration guide for changing existing mediable_type values

**Status**: ✅ Done (except the migration guide, deferred to the docs milestone). Default: `__schema__(:source)` (the Ecto table name, e.g. `"posts"`, `"blog_categories"`). Override via `use PhxMediaLibrary.HasMedia, media_type: "blog_posts"` or by defining `def __media_type__, do: "custom"`. Priority: 1) user-defined `__media_type__/0`, 2) explicit `:media_type` option, 3) Ecto table source, 4) underscored module name fallback. The `has_media()` macro, `MediaAdder`, `Media.get_mediable_info/1`, and `PhxMediaLibrary.get_mediable_type/1` all use the same resolution logic. Tests cover every derivation path.

**Why**: `"posts"` works for `Post`, but `"categorys"` for `Category` is wrong. Deriving from the Ecto table source (`__schema__(:source)`) is the most reliable default.
...
- [x] Nested `collection ... do convert ... end` syntax (v0.5.1), conversions auto-scoped to the enclosing collection
...
**Why**: this reads more naturally than returning lists from functions. It follows the Ecto DSL philosophy: declare your data model, don't configure it from afar. The nested style makes scoping visible. You can see at a glance which conversions apply to which collections.

---

## Milestone 3a: error handling and validation hardening (v0.3.0) ✅
...
- [x] Add the `:max_size` option to collection configuration
- [x] Validate file size before storage (not after)
- [x] Return a clear error: `{:error, {:file_too_large, size, max_size}}`
...
- [x] Reject files where content doesn't match the extension (configurable via the `:verify_content_type` collection option, default `true`)

### 3.4: batch operations

- [x] `clear_collection/2`, single query delete plus batch file removal (fixes N+1)
- [x] `clear_media/1`, single query delete plus batch file removal (fixes N+1)
- [x] `reorder/3`, `PhxMediaLibrary.reorder(post, :images, [id3, id1, id2])` using a single transaction
- [x] `move_to/2`, move a single media item to a position
- [x] Emit the `:media_reordered` Telemetry event

## Milestone 3b: remote URLs, metadata and Oban enhancements (v0.4.0) ✅

**Goal**: improve remote URL support, add automatic metadata extraction, and tighten Oban integration.
...
- [x] Custom request headers. `add_from_url/3` accepts a `:headers` option for authenticated downloads
- [x] Download timeout. The `:timeout` option sets a receive timeout for slow servers
- [x] Download telemetry. `[:phx_media_library, :download, :start | :stop | :exception]` events with URL, size, and MIME type metadata
- [x] Source URL tracking. The original URL is stored in `custom_properties["source_url"]`
- [x] Broader success codes. Downloads accept any 2xx status code (200 to 299), not just 200

### 3b.2: automatic metadata extraction

- [x] `PhxMediaLibrary.MetadataExtractor` behaviour with an `extract/3` callback
...
- [x] `metadata` JSON column on the Media schema (separate from `custom_properties`)
- [x] New migration `add_metadata_to_media` plus install task updated to include `metadata` from the start
- [x] Auto-extraction in the `to_collection/3` pipeline (after MIME detection, before storage)
...
- [x] Every extracted metadata includes an `"extracted_at"` ISO 8601 timestamp

### 3b.3: Oban conversion queuing (enhanced)

- [x] `process_sync/2`, a synchronous processing callback on `AsyncProcessor.Oban` for immediate conversions
- [x] Fuller documentation: setup walkthrough, queue sizing guidance, retry behaviour docs
...
- [x] Add an optional `deleted_at` field to the Media schema
...
- [x] Install task updated with the `deleted_at` column and index

### 3.6: streaming upload support (scoped)

- [x] Replace `File.read!` in `MediaAdder` with streaming from file to storage (64 KB chunks)
- [x] Compute the SHA-256 checksum during streaming in a single pass (not as a separate read)
- [x] Read only the first 512 bytes for MIME detection (magic bytes), not the entire file
- [x] All three storage adapters (Disk, Memory, S3) already handle `{:stream, stream}` content
- [x] Known issue "MediaAdder loads entire file into memory" is resolved
- [ ] **Deferred to M4**: full `{:stream, enumerable}` pipeline support for caller-provided streams

### 3.7: direct S3 upload (presigned URLs)

- [x] `presigned_upload_url/3` generates presigned URLs for client-to-S3 uploads
- [x] `complete_external_upload/4` creates a Media record after the client uploads directly to storage
- [x] `presigned_upload_url/3` callback added to the `PhxMediaLibrary.Storage` behaviour (optional)
- [x] The S3 adapter implements `presigned_upload_url/3` with `:expires_in`, `:content_type`, `:content_length_range` options
...
- [ ] **Deferred to M4**: a `<.media_upload>` variant that auto-detects S3 and uses external uploads
- [ ] **Deferred to M4**: LiveView external upload integration via the `allow_media_upload/3` `:external` option

---

## Milestone 4: best-in-class ✅

**Goal**: features that make this the library people reach for in the ecosystem.
...
- [x] Much smaller payload than base64 JPEG placeholders (roughly 25 bytes vs 500 bytes to 2 KB)
...
- [x] Generate a JPEG poster frame stored in `media.responsive_images["poster"]`
...
- [x] `path_context` escape hatch threads the tenant ID through any custom generator
- [x] Per-tenant storage configuration via the `:disk` option at upload time
...
| Async processing | Behaviour with Task (default) and Oban (optional) | Simple default, a stronger option when you need it |
...
|----------|---------|-------|
| Library name | `PhxMediaLibrary`, `PhxMedia`, `ExMedia`, `Mediator` | Parking for later. The `Phx` prefix implies Phoenix-only but the core is Ecto-only |

### Recently decided (Milestones 2 and v0.5.1)

| Decision | Choice | Rationale |
|----------|--------|-----------|
| Schema DSL style | Both: macro blocks (`media_collections do ... end`) plus function-based (`def media_collections, do: [...]`). Nested `collection ... do convert ... end` as the recommended style (v0.5.1) | The DSL reads more naturally; the nested style makes conversion scoping visible. Function style preserved for backwards compat. Mutually exclusive per concern but mixable. See 2.7 |
| Polymorphic type derivation | Ecto table name (`__schema__(:source)`) as default, overridable via a `use` option or `def __media_type__/0` | The table name is the most reliable default, no naive pluralization. Priority: user-defined fn, then explicit option, then table source, then module name fallback. See 2.6 |
| `has_many` injection | Via the `has_media()` macro inside the schema block, calling `Ecto.Schema.__has_many__/4` directly at module compilation time | Bypasses macro-expansion timing issues with Ecto's schema block. Uses `:where` for polymorphic scoping, `:defaults` for auto-populating. Collection-scoped variants via `has_media(:images)`. See 2.4 |
| LiveView component architecture | Three tiers: `MediaLive` LiveComponent (zero-boilerplate, v0.5.1), then `<.media_upload>` plus `<.media_gallery>` (composable), then `LiveUpload` helpers (full control) | `MediaLive` for a quick start; function components for layout control; the helper module for complete customization. See 1.0 to 1.3 |
...
| Model module discovery | `PhxMediaLibrary.ModelRegistry` (always compiled, v0.5.1) with a persistent_term cache | Decoupled from the Oban worker so mix tasks and non-Oban setups can discover model modules. See 2.3 |
| Optional image processing | `:image` marked `optional: true`; `ImageProcessor.Null` as a fallback with clear error messages; `Config.image_processor/0` auto-detects | The library works for file storage without libvips. See 2.1 |
| Checksum strategy | SHA-256 computed before storage, stored alongside the media record | Supports sha256/sha1/md5. `verify_integrity/1` re-reads and compares. See 2.5 |
...
- [x] **Oban worker creates empty Conversion structs.** Only the name is set, no dimensions/quality/format, so async processing produces broken conversions. (Fixed in Milestone 2.3)
- [x] **`:image` is a hard required dependency.** The library won't compile without libvips installed. (Fixed in Milestone 2.1)
- [x] **Pluralization is naive.** `get_mediable_type/1` just appends "s" to the module name, which breaks for irregular plurals. (Fixed in Milestone 2.6, now derives from the Ecto table name)
- [x] **`String.to_existing_atom/1` in `Config.disk_config/1`.** Crashes if the atom doesn't exist yet. (Fixed, now iterates disk keys and compares strings)
- [x] **No integration tests.** The core `to_collection/3` path is untested against a real database. (Fixed in Milestone 2.2, 56 integration tests)
- [x] **`has_media()` macro is a no-op.** It doesn't inject any association. (Fixed in Milestone 2.4, injects a polymorphic `has_many`)
- [x] **`clear_collection/2` and `clear_media/1` are N+1.** They fetch all, then delete one-by-one. (Fixed in Milestone 3.4, single `delete_all` query with batch file removal)
- [x] **`PathGenerator.full_path/2` uses `Keyword.keys(__info__(:functions))`.** A fragile way to check for an optional callback implementation. (Fixed, now uses `Code.ensure_loaded/1` plus `function_exported?/3`)
- [x] **`max_files` cleanup deletes newest items instead of oldest.** `Enum.drop(max)` on an ascending-ordered list removes the newest item. (Fixed, now keeps the newest `max` items and deletes the oldest excess)
...
- [x] **MIME detection is extension-only.** No content-based verification. (Fixed in Milestone 3.3, `MimeDetector` behaviour with a magic-bytes default, `:verify_content_type` collection option)
- [x] **`MediaAdder` loads the entire file into memory.** `File.read!` before storage. (Fixed in Milestone 3c/3.6, now streams in 64 KB chunks with a single-pass checksum)
- [x] **Roughly 15 noisy ExAws/S3 compile warnings in consumer projects.** `Storage.S3` was compiled unconditionally, producing `ExAws.S3.* is undefined` warnings for projects without S3 deps. (Fixed in v0.5.1, wrapped in `if Code.ensure_loaded?(ExAws.S3)`)
...
- [x] **Multi-file selection broken by default.** Non-`single_file` collections without `max_files` defaulted to `max_entries: 1` (the Phoenix LiveView default), which disabled the `multiple` attribute. (Fixed in v0.5.1, defaults to `max_entries: 10`)
- [x] **Upload progress bars invisible on fast/local uploads.** The progress bar only rendered when `progress > 0`, but local uploads complete instantly. (Fixed in v0.5.1, renders at `progress >= 0`)
...
- **Webhook/event system.** Beyond Telemetry, let users subscribe to media lifecycle events
